...
int ServerSocket(int port)
{
    ...
    if ((sEcoute = socket(AF_INET, SOCK_STREAM, 0)) == -1)
        //AF_Inet veut dire qu'on n'est sur un ipv4 et sock_stream veut modele TCP
	{
...
int Accept(int sEcoute,char *ipClient)// fonction blquant jusqu'a la connection d'un client 
{
	...
	int sService;
	if ((sService = accept(sEcoute,NULL,NULL)) == -1) // on ne recupere pas les donnees du client 
	{
	...
	// Recuperation d'information sur le client connecteµ
	...
	getpeername(sService,(struct sockaddr*)&adrClient,&adrClientLen);// recupere l'adresse ip du client 
...
int Receive(int sSocket,char* data)
{
    ...
    while(!fini)
    {
        if ((nbLus = read(sSocket,&lu1,1)) == -1) // le premier read est bloquant 
        {
            return -1;
        }
        if (nbLus == 0) 
